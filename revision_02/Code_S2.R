...
# ... it will run all the way through (~10 minutes on my computer)
...
ofm_hth_EDP_log2fd_plot <- "MerlinsManuscript/main/Figure_4.pdf"
...
ofs_hth_EDP_q1_sigs_tab <- "MerlinsManuscript/supplement/Table_S3.txt"
ofs_hth_EDP_alx_clr_plot <- "MerlinsManuscript/supplement/Figure_S3.pdf"
...
D_ofv_hth_EDP_q1_sigs <- "MerlinsManuscript/vault/table_EDP_q1_sigs.txt"
E_ofv_hth_EDP_log2fd_plot <- "MerlinsManuscript/vault/plot_EDP_log2fd.pdf"
F_ofv_timeline <- "MerlinsManuscript/vault/plot_timeline.pdf"
G_ofv_hth_EDP_alx_clr_plot <- "MerlinsManuscript/vault/plot_EDP_alx_clr.pdf"
...
G_ofv_WS <- "MerlinsManuscript/vault/WS_Section_G.RData"
...
# version info
# R 3.5.3 'Great Truth' (2019-03-11)
# R studio 1.1.463
# ALDEx2 1.14.1
# BiocParallel 1.16.6
# dplyr 0.8.1.1
...
# NOTE: these are not in the final, published plot
...
# relative paths from wd for outputs headed for storage in "supplement/"
ggsave(device = "pdf", dpi = 600, units = "mm", width = 165, height = 165,
...
# purpose:
# this section tests for differentially abundant features (ASVs) between...
...
D_hth_EDP_CvR_eff <- aldex.effect(D_hth_EDP_CvR_clr, useMC = TRUE,
                                  conditions = D_hth_EDP_CvR_cnd,
                                  include.sample.summary = TRUE)
D_hth_EDP_CvF_eff <- aldex.effect(D_hth_EDP_CvF_clr, useMC = TRUE,
                                  conditions = D_hth_EDP_CvF_cnd,
                                  include.sample.summary = TRUE)
D_hth_EDP_RvF_eff <- aldex.effect(D_hth_EDP_RvF_clr, useMC = TRUE,
                                  conditions = D_hth_EDP_RvF_cnd,
                                  include.sample.summary = TRUE)
...
# wi.eBH henceforth referred to as FDR-P value
...
# "Comparison", "log2 fold difference", "Direction of increase",	
# "P", "FDR-P", "Greengenes", "SILVA", "FeatureID"
...
# determine direction of increase (i.e. which diet group was ASV increased in?)
...
D_hth_EDP_CvR_q1_sigs_4 <- dplyr::select(D_hth_EDP_CvR_q1_sigs_3, 
                                         Comparison,
                                         log2.fold.difference = diff.btw,
                                         Direction.of.increase, 
                                         P = wi.ep, FDR_P = wi.eBH, 
...
# print the number of differentially abundundant ASVs from above
...
write.table(sep = "\t", row.names = FALSE, x = D_EDP_q1_sigs,
            file = ofs_hth_EDP_q1_sigs_tab)

# relative paths for outputs headed for storage in the "vault/"
write.table(sep = "\t", row.names = FALSE, x = D_EDP_q1_sigs,
            file = D_ofv_hth_EDP_q1_sigs)
...
### ************************************
### SECTION E - visualize features of interest (main) ----
### ************************************

# purpose:
# this section creates ggplots for differentially abundant ASVs... 
# of potential interest (visualized as log2 fold differences)

# main/supplementary items generated:
# Figure 4

# NOTE: in this section, FeatureIDs are explicilty referenced
# if starting from Code_S1.R which means you ran through QIIME
# and you may not have the same FeatureIDs
...
# create a new version of relevant item from Section D
E_EDP_q1_sigs_1 <- D_EDP_q1_sigs_1

# filter to isolate features of interest
E_fts_flt <- dplyr::filter(E_EDP_q1_sigs_1,
                           FeatureID == "2d0b61d3962846ad597598bd8657bdc3" |
                             ...
                             FeatureID == "dd599b06e0976f4b08f6ca7b6a194e36" |
                             FeatureID == "82434283f3ab53e959d51322ae902892" |
                             FeatureID == "fa015d48a85d320c7084a2744186d4b3" |
                             FeatureID == "d4145a4b797c890b1f5b38c3f66ea85b" |
                             FeatureID == "da402f9e0542704e361381a2df159e4d" |
                             FeatureID == "2e028bb632768e35b6f425e8202b5ecf" |
                             FeatureID == "99e03d98be225fc4699960359c8c86b5" |
                             FeatureID == "876bbabcbca736f299975a3bdd4dcb00" |
                             FeatureID == "97045c49b1ff5340af3f820f6fa5b3d8" |
                             FeatureID == "ba84df5178b7257a2863c00f1e6866db")

# filter to isolate pairwise comparisons
E_RvC_0 <- dplyr::filter(E_fts_flt, Comparison == "R vs C")
E_FvC_0 <- dplyr::filter(E_fts_flt, Comparison == "F vs C")
E_RvF_0 <- dplyr::filter(E_fts_flt, Comparison == "R vs F")

# simplify the above dfs for plotting
E_RvC_1 <- dplyr::select(E_RvC_0, Comparison,
                         group = Direction.of.increase,
                         log2fd = log2.fold.difference,
                         Greengenes, SILVA, FeatureID)

E_FvC_1 <- dplyr::select(E_FvC_0, Comparison,
                         group = Direction.of.increase,
                         log2fd = log2.fold.difference,
                         Greengenes, SILVA, FeatureID)

E_RvF_1 <- dplyr::select(E_RvF_0, Comparison,
                         group = Direction.of.increase,
                         log2fd = log2.fold.difference,
                         Greengenes, SILVA, FeatureID)

# define order to plot on x axis
E_RvC_FvC_ord <- c("2d0b61d3962846ad597598bd8657bdc3",
                   "2d0559e9488391eb0918ff0a97e14037", 
                   "73f14d03d440b253e6e671db77fb578a", 
                   "f03b21b0d3b38bc8b7ab248a02d1073e", 
                   "15ac133af11f957ad7bf0943b14a4993", 
                   "243a725661674fde1c0c2afd5f24393d", 
                   "d4145a4b797c890b1f5b38c3f66ea85b",
                   "da402f9e0542704e361381a2df159e4d",
                   "fa015d48a85d320c7084a2744186d4b3",
                   "dd599b06e0976f4b08f6ca7b6a194e36",
                   "82434283f3ab53e959d51322ae902892",
                   "876bbabcbca736f299975a3bdd4dcb00",
                   "97045c49b1ff5340af3f820f6fa5b3d8", 
                   "2e028bb632768e35b6f425e8202b5ecf",
                   "99e03d98be225fc4699960359c8c86b5",
                   "ba84df5178b7257a2863c00f1e6866db")

E_RvF_ord <- c("083e6b5d9d344047f0553b3052e0ccf3",
               "6bc1c99d965f0989dfcfcfeacf57ccc7")

# sort dfs using the above order
E_RvC_2 <- E_RvC_1[order(match(E_RvC_1$FeatureID, E_RvC_FvC_ord)), ]
E_FvC_2 <- E_FvC_1[order(match(E_FvC_1$FeatureID, E_RvC_FvC_ord)), ]
E_RvF_2 <- E_RvF_1[order(match(E_RvF_1$FeatureID, E_RvF_ord)), ]

# we will use the lowest assignment for the SILVA database for x-axis labels
# to clarify instances where ggs and slv matched, add ' to end of taxon name
E_RvC_3 <- E_RvC_2
E_FvC_3 <- E_FvC_2
E_RvF_3 <- E_RvF_2

# RvC
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "2d0b61d3962846ad597598bd8657bdc3"] <- "Roseburia"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "2d0559e9488391eb0918ff0a97e14037"] <- "Clostridiales"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "73f14d03d440b253e6e671db77fb578a"] <- "Lachnospiraceae'"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "f03b21b0d3b38bc8b7ab248a02d1073e"] <- "Lachnospiraceae"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "15ac133af11f957ad7bf0943b14a4993"] <- "Lachnospiraceae"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "243a725661674fde1c0c2afd5f24393d"] <- "Lachnospiraceae"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "d4145a4b797c890b1f5b38c3f66ea85b"] <- "Blautia"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "da402f9e0542704e361381a2df159e4d"] <- "Lachnoclostridium"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "fa015d48a85d320c7084a2744186d4b3"] <- "Ruminoclostridium"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "dd599b06e0976f4b08f6ca7b6a194e36"] <- "Lactococcus'"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "82434283f3ab53e959d51322ae902892"] <- "Dorea'"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "876bbabcbca736f299975a3bdd4dcb00"] <- "Lachnospiraceae"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "97045c49b1ff5340af3f820f6fa5b3d8"] <- "Lachnospiraceae"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "2e028bb632768e35b6f425e8202b5ecf"] <- "Lachnospiraceae'"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "99e03d98be225fc4699960359c8c86b5"] <- "Lachnospiraceae'"
E_RvC_3$tax[E_RvC_3$FeatureID == 
              "ba84df5178b7257a2863c00f1e6866db"] <- "Ruminococcaceae"

# FvC
E_FvC_3$tax[E_FvC_3$FeatureID == 
              "2d0b61d3962846ad597598bd8657bdc3"] <- "Roseburia"
E_FvC_3$tax[E_FvC_3$FeatureID == 
              "2d0559e9488391eb0918ff0a97e14037"] <- "Clostridiales"
E_FvC_3$tax[E_FvC_3$FeatureID == 
...
# RvF
E_RvF_3$tax[E_RvF_3$FeatureID == 
              "083e6b5d9d344047f0553b3052e0ccf3"] <- "Clostridiales'"
E_RvF_3$tax[E_RvF_3$FeatureID == 
              "6bc1c99d965f0989dfcfcfeacf57ccc7"] <- "Roseburia"
...
# hex codes for diet group
E_hex_CFR <- c("#33a02c", "#e66101", "#1f78b4")

# font family
E_fnt_fam <- "Courier"

# font faces to specify italics in RvC and FvC
E_fce <- c("italic", "plain", rep("italic", times = 14))

# save parameters
E_lab_fnt <- list(size = 11, family = "Courier", face = "bold")
...
# create dfs for plotting group labels
E_lab_RvC <- data.frame(x = c(3.5, 11.5), y = c(-0.8, 0.8),
                        label = c("Rice bran (R)", "Control (C)"),
                        color = c(E_hex_CFR[3], E_hex_CFR[1]))

E_lab_FvC <- data.frame(x = c(3.5, 11.5), y = c(-0.8, 0.8),
                        color = c(E_hex_CFR[2], E_hex_CFR[1]))

E_lab_RvF <- data.frame(x = c(1, 2), y = c(-0.8, 0.8), label = c("(R)", "(F)"),
                        color = c(E_hex_CFR[3], E_hex_CFR[2]))
...
# RvC panel A
E_ggp_RvC <- ggbarplot(data = E_RvC_3, x = "FeatureID", y = "log2fd",
                       color = "white", fill = "group", width = 0.8,
                       font.family = E_fnt_fam,
                       palette = c(E_hex_CFR[1], E_hex_CFR[3])) +
  border(color = E_blk) +
  geom_text(data = E_lab_RvC, aes(x = x, y = y, label = label), 
            color = E_lab_RvC$color, family = E_fnt_fam, size = E_grp_sze) +
  geom_text(data = E_lab_RvC, aes(x = x, y = y, label = label), 
            color = E_lab_RvC$color, family = E_fnt_fam, size = E_grp_sze) +
  geom_text(data = E_lab_RvC, aes(x = x, y = y, label = label), 
            color = E_lab_RvC$color, family = E_fnt_fam, size = E_grp_sze) +
  theme(axis.text.x = element_text(size = E_axs_x, face = E_fce,
                                   angle = 90, hjust = 1, vjust = 0.5),
        axis.text.y = element_text(size = E_axs_y),
        axis.title = element_blank(),
        legend.position = "none",
        line = element_line(lineend = "square"),
        panel.ontop = T,
        panel.background = element_rect(fill = NA)) +
  scale_x_discrete(labels = E_RvC_3$tax, expand = c(0.05, 0)) +
  scale_y_continuous(limits = E_lim_y, breaks = E_brk_y, labels = E_lab_y, 
                     expand = c(0, 0))

# FvC panel B
E_ggp_FvC <- ggbarplot(data = E_FvC_3, x = "FeatureID", y = "log2fd",
                       color = "white", fill = "group", width = 0.8,
                       font.family = E_fnt_fam,
                       palette = c(E_hex_CFR[2], E_hex_CFR[1])) +
  border(color = E_blk) +
  geom_text(data = E_lab_FvC, aes(x = x, y = y), color = E_lab_FvC$color, 
            family = E_fnt_fam, size = E_grp_sze,
            label = c(expression(paste(italic("B.longum"), "-Fermented (F)")), 
                      "Control (C)")) +
  geom_text(data = E_lab_FvC, aes(x = x, y = y), color = E_lab_FvC$color, 
            family = E_fnt_fam, size = E_grp_sze,
            label = c(expression(paste(italic("B.longum"), "-Fermented (F)")), 
  ...
  theme(axis.text.x = element_text(size = E_axs_x, face = E_fce,
        ...
        panel.ontop = T,
        panel.background = element_rect(fill = NA)) +
  scale_x_discrete(labels = E_FvC_3$tax, expand = c(0.05, 0)) +
  scale_y_continuous(limits = E_lim_y, breaks = E_brk_y, labels = E_lab_y, 
                     expand = c(0, 0))

# RvF panel C
E_ggp_RvF <- ggbarplot(data = E_RvF_3, x = "FeatureID", y = "log2fd",
                       color = "white", fill = "group", width = 0.8,
                       font.family = E_fnt_fam,
                       palette = c(E_hex_CFR[2], E_hex_CFR[3])) +
  border(color = E_blk) +
  geom_text(data = E_lab_RvF, aes(x = x, y = y, label = label), 
            color = E_lab_RvF$color, family = E_fnt_fam, size = E_grp_sze) +
  geom_text(data = E_lab_RvF, aes(x = x, y = y, label = label), 
            color = E_lab_RvF$color, family = E_fnt_fam, size = E_grp_sze) +
  geom_text(data = E_lab_RvF, aes(x = x, y = y, label = label), 
            color = E_lab_RvF$color, family = E_fnt_fam, size = E_grp_sze) +
  theme(axis.text.x = element_text(size = E_axs_x, face = c("plain", "italic"),
                                   angle = 90, hjust = 1, vjust = 0.5),
        axis.text.y = element_text(size = E_axs_y),
        axis.title = element_blank(),
        legend.position = "none",
        ...
        panel.ontop = T,
        panel.background = element_rect(fill = NA)) +
  scale_x_discrete(labels = E_RvF_3$tax, expand = c(0.75, 0)) +
  scale_y_continuous(limits = E_lim_y, breaks = E_brk_y, labels = E_lab_y, 
                     expand = c(0, 0))

### ************************************
### E - STEP 4 - arrange ggplots into panels ----
### ************************************

# arrange
E_gga <- ggarrange(E_ggp_RvC, E_ggp_FvC, E_ggp_RvF, hjust = 0.05,
                   labels = "AUTO", font.label = E_lab_fnt,
                   ncol = 3, nrow = 1, align = "hv", widths = c(1, 1, 0.256))

# add universal y axis label
E_ylab <- ggpubr::text_grob("log2 fold difference", color = E_blk, size = 5,
                            family = E_fnt_fam, rot = 90, 
                            hjust = -0.02, vjust = 0.25)

E_gga_plot <- ggpubr::annotate_figure(E_gga, left = E_ylab)
...
# relative paths from wd for outputs headed for storage in "main/"
ggsave(device = "pdf", dpi = 600, units = "mm", width = 165, height = 55,
       filename = ofm_hth_EDP_log2fd_plot, plot = E_gga_plot)

# relative paths for outputs headed for storage in the "vault/"
ggsave(device = "pdf", dpi = 600, units = "mm", width = 165, height = 55,
       filename = E_ofv_hth_EDP_log2fd_plot, plot = E_gga_plot)

# create a list of objects from section and save them as a workspace file
E_obj <- c(ls(pattern = "E_"), "D_EDP_q1_sigs_1")
...
### ************************************
### SECTION G - visualize features of interest (supp) ----
### ************************************

# purpose:
# this section creates ggplots for differentially abundant ASVs... 
# of potential interest (visualized as each sample's clr value)

# main/supplementary items generated:
# Figure S3

# NOTE: in this sectin, FeatureIDs are explicilty referenced
# if starting from Code_S1.R which means you ran through QIIME
# and you may not have the same FeatureIDs
# as a result, the code in this section WILL NOT work
# it is written in my notes to alter this in later versions; however,
...
### ************************************
### G - STEP 1 - format ALDEx2 results for ggplot ----
### ************************************

# create new versions of relefvant items from Section D
G_RvC_q1_sigs_0 <- D_hth_EDP_CvR_q1_sigs_0
G_FvC_q1_sigs_0 <- D_hth_EDP_CvF_q1_sigs_0
G_RvF_q1_sigs_0 <- D_hth_EDP_RvF_q1_sigs_0

# reorder, rename, and drop unneeded columns
G_RvC_q1_sigs_1 <- dplyr::select(G_RvC_q1_sigs_0, 
                                 FeatureID, rab.win.C, rab.win.R, 
                                 dplyr::contains("sample"), 
                                 ggs.txn = int.ggs.lws.txn, 
                                 slv.txn = int.slv.lws.txn)

G_FvC_q1_sigs_1 <- dplyr::select(G_FvC_q1_sigs_0, 
                                 FeatureID, rab.win.C, rab.win.F, 
                                 dplyr::contains("sample"), 
                                 ggs.txn = int.ggs.lws.txn, 
                                 slv.txn = int.slv.lws.txn)

G_RvF_q1_sigs_1 <- dplyr::select(G_RvF_q1_sigs_0, 
                                 FeatureID, rab.win.R, rab.win.F, 
                                 dplyr::contains("sample"), 
                                 ggs.txn = int.ggs.lws.txn, 
                                 slv.txn = int.slv.lws.txn)

# filter to isolate features of interest
G_RvC_fts_flt <- dplyr::filter(
  G_RvC_q1_sigs_1,
  FeatureID == "2d0b61d3962846ad597598bd8657bdc3" |
    FeatureID == "73f14d03d440b253e6e671db77fb578a" |
    FeatureID == "f03b21b0d3b38bc8b7ab248a02d1073e" |
...
G_RvC_grp_0 <- dplyr::select(G_RvC_fts_flt, -dplyr::contains("sample"))
G_FvC_grp_0 <- dplyr::select(G_FvC_fts_flt, -dplyr::contains("sample"))
G_RvF_grp_0 <- dplyr::select(G_RvF_q1_sigs_1, -dplyr::contains("sample"))

G_RvC_smp_0 <- dplyr::select(G_RvC_fts_flt, -dplyr::contains("win"))
G_FvC_smp_0 <- dplyr::select(G_FvC_fts_flt, -dplyr::contains("win"))
G_RvF_smp_0 <- dplyr::select(G_RvF_q1_sigs_1, -dplyr::contains("win"))

# format dfs to remove 'rab.win.' and 'rab.sample.'
G_RvC_grp_1 <- G_RvC_grp_0
G_FvC_grp_1 <- G_FvC_grp_0
G_RvF_grp_1 <- G_RvF_grp_0

G_RvC_smp_1 <- G_RvC_smp_0
G_FvC_smp_1 <- G_FvC_smp_0
G_RvF_smp_1 <- G_RvF_smp_0

names(G_RvC_grp_1) <- gsub(pattern = "rab.win.", replacement = "", 
                           x = names(G_RvC_grp_1))
names(G_FvC_grp_1) <- gsub(pattern = "rab.win.", replacement = "", 
                           x = names(G_FvC_grp_1))
names(G_RvF_grp_1) <- gsub(pattern = "rab.win.", replacement = "", 
                           x = names(G_RvF_grp_1))

names(G_RvC_smp_1) <- gsub(pattern = "rab.sample.", replacement = "", 
                           x = names(G_RvC_smp_1))
names(G_FvC_smp_1) <- gsub(pattern = "rab.sample.", replacement = "", 
                           x = names(G_FvC_smp_1))
names(G_RvF_smp_1) <- gsub(pattern = "rab.sample.", replacement = "", 
                           x = names(G_RvF_smp_1))

# reshape dfs to convert columns with sample information into rows...
# ... with correseponding column data for each sample
G_id_vars <- c("FeatureID", "ggs.txn", "slv.txn")

G_RvC_grp_2 <- reshape2::melt(G_RvC_grp_1, id.vars = G_id_vars, 
                              variable.name = "DietOneLetter", 
                              value.name = "med.clr")
G_FvC_grp_2 <- reshape2::melt(G_FvC_grp_1, id.vars = G_id_vars, 
                              variable.name = "DietOneLetter", 
                              value.name = "med.clr")
G_RvF_grp_2 <- reshape2::melt(G_RvF_grp_1, id.vars = G_id_vars, 
                              variable.name = "DietOneLetter", 
                              value.name = "med.clr")

G_RvC_smp_2 <- reshape2::melt(G_RvC_smp_1, id.vars = G_id_vars,, 
                              variable.name = "DietID", 
                              value.name = "med.clr")
G_FvC_smp_2 <- reshape2::melt(G_FvC_smp_1, id.vars = G_id_vars,, 
                              variable.name = "DietID", 
                              value.name = "med.clr")
G_RvF_smp_2 <- reshape2::melt(G_RvF_smp_1, id.vars = G_id_vars,, 
                              variable.name = "DietID", 
                              value.name = "med.clr")

# read in metadata file, retain needed columns, and merge w/ reshaped sample dfs
G_met_0 <- read.table(ifp_met, header = T, sep = "\t", as.is = T)

G_met_1 <- dplyr::select(G_met_0, DietID, DietOneLetter, TypeOneLetter)

G_RvC_smp_met_0 <- merge(x = G_met_1, y = G_RvC_smp_2, by = "DietID",
                         all = F, sort = T)
G_FvC_smp_met_0 <- merge(x = G_met_1, y = G_FvC_smp_2, by = "DietID",
                         all = F, sort = T)
G_RvF_smp_met_0 <- merge(x = G_met_1, y = G_RvF_smp_2, by = "DietID",
                         all = F, sort = T)

# define order to plot on x axis
# define order to plot on x axis
G_RvC_FvC_ord <- c("2d0b61d3962846ad597598bd8657bdc3",
                   "2d0559e9488391eb0918ff0a97e14037", 
                   "73f14d03d440b253e6e671db77fb578a", 
                   "f03b21b0d3b38bc8b7ab248a02d1073e", 
                   "15ac133af11f957ad7bf0943b14a4993", 
...
G_RvF_ord <- c("083e6b5d9d344047f0553b3052e0ccf3",
               "6bc1c99d965f0989dfcfcfeacf57ccc7")

# sort smp_met and grp dfs using the above order
G_RvC_smp_met_1 <- G_RvC_smp_met_0[order(match(G_RvC_smp_met_0$FeatureID, 
                                               G_RvC_FvC_ord)), ]
G_FvC_smp_met_1 <- G_FvC_smp_met_0[order(match(G_FvC_smp_met_0$FeatureID, 
                                               G_RvC_FvC_ord)), ]
G_RvF_smp_met_1 <- G_RvF_smp_met_0[order(match(G_RvF_smp_met_0$FeatureID, 
                                               G_RvF_ord)), ]

G_RvC_grp_3 <- G_RvC_grp_2[order(match(G_RvC_grp_2$FeatureID, G_RvC_FvC_ord)), ]
G_FvC_grp_3 <- G_FvC_grp_2[order(match(G_FvC_grp_2$FeatureID, G_RvC_FvC_ord)), ]
G_RvF_grp_3 <- G_RvF_grp_2[order(match(G_RvF_grp_2$FeatureID, G_RvF_ord)), ]

# we will use the lowest assignment for the SILVA database for x-axis labels
# to clarify instances where ggs and slv matched, add ' to end of taxon name
G_RvC_smp_met_2 <- G_RvC_smp_met_1
G_FvC_smp_met_2 <- G_FvC_smp_met_1
G_RvF_smp_met_2 <- G_RvF_smp_met_1

# RvC
G_RvC_smp_met_2$tax[G_RvC_smp_met_2$FeatureID == 
                      "2d0b61d3962846ad597598bd8657bdc3"] <- "Roseburia"
G_RvC_smp_met_2$tax[G_RvC_smp_met_2$FeatureID == 
                      "2d0559e9488391eb0918ff0a97e14037"] <- "Clostridiales"
G_RvC_smp_met_2$tax[G_RvC_smp_met_2$FeatureID == 
...
# FvC
G_FvC_smp_met_2$tax[G_FvC_smp_met_2$FeatureID == 
                      "2d0b61d3962846ad597598bd8657bdc3"] <- "Roseburia"
G_FvC_smp_met_2$tax[G_FvC_smp_met_2$FeatureID == 
                      "2d0559e9488391eb0918ff0a97e14037"] <- "Clostridiales"
G_FvC_smp_met_2$tax[G_FvC_smp_met_2$FeatureID == 
...
G_FvC_smp_met_2$tax[G_FvC_smp_met_2$FeatureID == 
                      "ba84df5178b7257a2863c00f1e6866db"] <- "Ruminococcaceae"
# RvF
G_RvF_smp_met_2$tax[G_RvF_smp_met_2$FeatureID == 
                      "083e6b5d9d344047f0553b3052e0ccf3"] <- "Clostridiales'"
G_RvF_smp_met_2$tax[G_RvF_smp_met_2$FeatureID == 
                      "6bc1c99d965f0989dfcfcfeacf57ccc7"] <- "Roseburia"

# now format the grp dfs to be plotted as segments
G_gren <- "#33a02c"
G_blue <- "#1f78b4"
G_ornj <- "#e66101"
G_dash <- "dashed"
...
G_RvC_grp_4 <- G_RvC_grp_3
G_FvC_grp_4 <- G_FvC_grp_3
G_RvF_grp_4 <- G_RvF_grp_3

G_RvC_grp_4$color[G_RvC_grp_4$DietOneLetter == "R"] <- G_blue
G_RvC_grp_4$color[G_RvC_grp_4$DietOneLetter == "C"] <- G_gren
G_RvC_grp_4$line[G_RvC_grp_4$DietOneLetter == "R"] <- G_dash
G_RvC_grp_4$line[G_RvC_grp_4$DietOneLetter == "C"] <- G_dotd

G_FvC_grp_4$color[G_FvC_grp_4$DietOneLetter == "F"] <- G_ornj
G_FvC_grp_4$color[G_FvC_grp_4$DietOneLetter == "C"] <- G_gren
G_FvC_grp_4$line[G_FvC_grp_4$DietOneLetter == "F"] <- G_sold
G_FvC_grp_4$line[G_FvC_grp_4$DietOneLetter == "C"] <- G_dotd

G_RvF_grp_4$color[G_RvF_grp_4$DietOneLetter == "R"] <- G_blue
G_RvF_grp_4$color[G_RvF_grp_4$DietOneLetter == "F"] <- G_ornj
G_RvF_grp_4$line[G_RvF_grp_4$DietOneLetter == "R"] <- G_dash
G_RvF_grp_4$line[G_RvF_grp_4$DietOneLetter == "F"] <- G_sold

# define x axis scale as continuous (numeric), this allows us to... 
# ... dodge our points AND segments, but it's a bit of code nightmare
G_RvC_smp_met_3 <- G_RvC_smp_met_2
G_FvC_smp_met_3 <- G_FvC_smp_met_2
G_RvF_smp_met_3 <- G_RvF_smp_met_2

G_RvC_grp_5 <- G_RvC_grp_4
G_FvC_grp_5 <- G_FvC_grp_4
G_RvF_grp_5 <- G_RvF_grp_4

# RvC
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "2d0b61d3962846ad597598bd8657bdc3"] <- 1
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "2d0559e9488391eb0918ff0a97e14037"] <- 2
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "73f14d03d440b253e6e671db77fb578a"] <- 3
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "f03b21b0d3b38bc8b7ab248a02d1073e"] <- 4
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "15ac133af11f957ad7bf0943b14a4993"] <- 5
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "243a725661674fde1c0c2afd5f24393d"] <- 6
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "d4145a4b797c890b1f5b38c3f66ea85b"] <- 7
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "da402f9e0542704e361381a2df159e4d"] <- 8
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "fa015d48a85d320c7084a2744186d4b3"] <- 9
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "dd599b06e0976f4b08f6ca7b6a194e36"] <- 10
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "82434283f3ab53e959d51322ae902892"] <- 11
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "876bbabcbca736f299975a3bdd4dcb00"] <- 12
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "97045c49b1ff5340af3f820f6fa5b3d8"] <- 13
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "2e028bb632768e35b6f425e8202b5ecf"] <- 14
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "99e03d98be225fc4699960359c8c86b5"] <- 15
G_RvC_smp_met_3$scale[G_RvC_smp_met_3$FeatureID == 
                        "ba84df5178b7257a2863c00f1e6866db"] <- 16

G_RvC_grp_5$scale[G_RvC_grp_5$FeatureID == 
                    "2d0b61d3962846ad597598bd8657bdc3"] <- 1
G_RvC_grp_5$scale[G_RvC_grp_5$FeatureID == 
                    "2d0559e9488391eb0918ff0a97e14037"] <- 2
G_RvC_grp_5$scale[G_RvC_grp_5$FeatureID == 
...
# FvC
G_FvC_smp_met_3$scale[G_FvC_smp_met_3$FeatureID == 
                        "2d0b61d3962846ad597598bd8657bdc3"] <- 1
G_FvC_smp_met_3$scale[G_FvC_smp_met_3$FeatureID == 
                        "2d0559e9488391eb0918ff0a97e14037"] <- 2
G_FvC_smp_met_3$scale[G_FvC_smp_met_3$FeatureID == 
...
G_FvC_grp_5$scale[G_FvC_grp_5$FeatureID == 
                    "2d0b61d3962846ad597598bd8657bdc3"] <- 1
G_FvC_grp_5$scale[G_FvC_grp_5$FeatureID == 
                    "2d0559e9488391eb0918ff0a97e14037"] <- 2
G_FvC_grp_5$scale[G_FvC_grp_5$FeatureID == 
...
# RvF
G_RvF_smp_met_3$scale[G_RvF_smp_met_3$FeatureID == 
                        "083e6b5d9d344047f0553b3052e0ccf3"] <- 1
G_RvF_smp_met_3$scale[G_RvF_smp_met_3$FeatureID == 
                        "6bc1c99d965f0989dfcfcfeacf57ccc7"] <- 2

G_RvF_grp_5$scale[G_RvF_grp_5$FeatureID == 
                    "083e6b5d9d344047f0553b3052e0ccf3"] <- 1
G_RvF_grp_5$scale[G_RvF_grp_5$FeatureID == 
                    "6bc1c99d965f0989dfcfcfeacf57ccc7"] <- 2

# now dodge using scaler based upon diet group
G_dodge <- 0.15

G_RvC_smp_met_4 <- G_RvC_smp_met_3
G_FvC_smp_met_4 <- G_FvC_smp_met_3
G_RvF_smp_met_4 <- G_RvF_smp_met_3

G_RvC_grp_6 <- G_RvC_grp_5
G_FvC_grp_6 <- G_FvC_grp_5
G_RvF_grp_6 <- G_RvF_grp_5

# RvC
G_RvC_smp_met_4$dodge <- ifelse(G_RvC_smp_met_4$DietOneLetter == "R", 
                                y = G_RvC_smp_met_4$scale - G_dodge,
                                n = G_RvC_smp_met_4$scale + G_dodge)

G_RvC_grp_6$dodge <- ifelse(G_RvC_grp_6$DietOneLetter == "R", 
                            y = G_RvC_grp_6$scale - G_dodge,
                            n = G_RvC_grp_6$scale + G_dodge)

# FvC
G_FvC_smp_met_4$dodge <- ifelse(G_FvC_smp_met_4$DietOneLetter == "F", 
                                y = G_FvC_smp_met_4$scale - G_dodge,
                                n = G_FvC_smp_met_4$scale + G_dodge)

G_FvC_grp_6$dodge <- ifelse(G_FvC_grp_6$DietOneLetter == "F", 
                            y = G_FvC_grp_6$scale - G_dodge,
                            n = G_FvC_grp_6$scale + G_dodge)

# RvF
G_RvF_smp_met_4$dodge <- ifelse(G_RvF_smp_met_4$DietOneLetter == "R", 
                                y = G_RvF_smp_met_4$scale - G_dodge,
                                n = G_RvF_smp_met_4$scale + G_dodge)

G_RvF_grp_6$dodge <- ifelse(G_RvF_grp_6$DietOneLetter == "R", 
                            y = G_RvF_grp_6$scale - G_dodge,
                            n = G_RvF_grp_6$scale + G_dodge)

# define x-axis labels and breaks
G_RvC_xlab <- c(unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 1]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 2]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 3]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 4]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 5]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 6]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 7]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 8]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 9]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 10]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 11]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 12]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 13]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 14]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 15]),
                unique(G_RvC_smp_met_4$tax[G_RvC_smp_met_4$scale == 16]))

G_FvC_xlab <- c(unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 1]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 2]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 3]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 4]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 5]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 6]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 7]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 8]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 9]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 10]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 11]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 12]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 13]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 14]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 15]),
                unique(G_FvC_smp_met_4$tax[G_FvC_smp_met_4$scale == 16]))

G_RvF_xlab <- c(unique(G_RvF_smp_met_4$tax[G_RvF_smp_met_4$scale == 1]),
                unique(G_RvF_smp_met_4$tax[G_RvF_smp_met_4$scale == 2]))

G_RvC_xbrk <- (unique(G_RvC_smp_met_4$scale))
G_FvC_xbrk <- (unique(G_FvC_smp_met_4$scale))
G_RvF_xbrk <- (unique(G_RvF_smp_met_4$scale))

### ************************************
### G - STEP 2 - format for ggplots ----
### ************************************
...
# hex codes for diet group
G_hex_CFR <- c("#33a02c", "#e66101", "#1f78b4")

# shapes for sample type
G_shp_typ <- c(19, 17, 1) # plot order is: D E P

# font family
G_fnt_fam <- "Courier"

# font faces to specify italics in RvC and FvC
G_fce <- c("italic", "plain", rep("italic", times = 14))
G_RvF_fce <- c("plain", "italic")

# save parameters
G_lab_fnt <- list(size = 11, family = "Courier", face = "bold")
...
# y-axis parameters
G_lim_y <- c(-4.4, 13)
G_brk_y <- c(-3, 0, 3, 6, 9, 12)
G_lab_y <- c("-3", "0", "3", "6", "9", "12")

# scaler for dodging segments
G_seg_scale <- 0.235

# jitter for points
G_jit <- 0.05
G_pos_jit <- position_jitter(width = G_jit, seed = 123456789)

### ************************************
### G - STEP 3 - create ggplots ----
### ************************************

# NOTE: lines (segments) for groups plotted twice to darken

# RvC panel A
G_ggp_RvC <- ggplot(data = G_RvC_smp_met_4, aes(x = dodge, y = med.clr,
                                                color = DietOneLetter, 
                                                shape = TypeOneLetter)) +
  labs(y = "clr abundance") +
  geom_vline(color = G_gry, size = G_vln, linetype = 1, xintercept = 6.5) +
  geom_point(size = G_pts, stroke = G_stk, alpha = 0.42, position = G_pos_jit) +
  geom_segment(inherit.aes = F, data = G_RvC_grp_6, size = G_seg,
               color = G_RvC_grp_5$color, linetype = G_RvC_grp_5$line, 
               aes(x = dodge + G_seg_scale, xend = dodge - G_seg_scale, 
                   y = med.clr, yend = med.clr)) +
  geom_segment(inherit.aes = F, data = G_RvC_grp_6, size = G_seg,
               color = G_RvC_grp_5$color, linetype = G_RvC_grp_5$line, 
               aes(x = dodge + G_seg_scale, xend = dodge - G_seg_scale, 
                   y = med.clr, yend = med.clr)) +
  scale_color_manual(values = c(G_hex_CFR[1], G_hex_CFR[3])) +
  scale_shape_manual(values = G_shp_typ) + 
  theme_pubr() +
  border(color = G_blk) +
  theme(line = element_line(lineend = "square"),
        text = element_text(family = G_fnt_fam),
        axis.text.y = element_text(size = G_axs_y, face = G_fce),
        axis.text.x = element_text(size = G_axs_x),
        axis.title.y = element_blank(),
        axis.title.x = element_text(size = G_ttl_x),
        legend.position = "none",
        panel.ontop = T,
        panel.background = element_rect(fill = NA)) +
  scale_x_continuous(breaks = G_RvC_xbrk, labels = G_RvC_xlab, 
                     expand = c(0.01, 0)) +
  scale_y_continuous(limits = G_lim_y, breaks = G_brk_y, labels = G_lab_y, 
                     expand = c(0, 0)) +
  coord_flip()

# FvC panel B
G_ggp_FvC <- ggplot(data = G_FvC_smp_met_4, aes(x = dodge, y = med.clr,
                                                color = DietOneLetter, 
                                                shape = TypeOneLetter)) +
  labs(y = "clr abundance") +
  geom_vline(color = G_gry, size = G_vln, linetype = 1, xintercept = 6.5) +
  geom_point(size = G_pts, stroke = G_stk, alpha = 0.42, position = G_pos_jit) +
  geom_segment(inherit.aes = F, data = G_FvC_grp_6, size = G_seg,
               color = G_FvC_grp_5$color, linetype = G_FvC_grp_5$line, 
               aes(x = dodge + G_seg_scale, xend = dodge - G_seg_scale, 
                   y = med.clr, yend = med.clr)) +
  geom_segment(inherit.aes = F, data = G_FvC_grp_6, size = G_seg,
               color = G_FvC_grp_5$color, linetype = G_FvC_grp_5$line, 
               aes(x = dodge + G_seg_scale, xend = dodge - G_seg_scale, 
                   y = med.clr, yend = med.clr)) +
  scale_color_manual(values = c(G_hex_CFR[1], G_hex_CFR[2])) +
  scale_shape_manual(values = G_shp_typ) + 
  theme_pubr() +
  border(color = G_blk) +
  theme(line = element_line(lineend = "square"),
        ...
        panel.ontop = T,
        panel.background = element_rect(fill = NA)) +
  scale_x_continuous(breaks = G_FvC_xbrk, labels = G_FvC_xlab, 
                     expand = c(0.01, 0)) +
  scale_y_continuous(limits = G_lim_y, breaks = G_brk_y, labels = G_lab_y, 
                     expand = c(0, 0)) +
  coord_flip()
...
# RvF panel C
G_ggp_RvF <- ggplot(data = G_RvF_smp_met_4, aes(x = dodge, y = med.clr,
                                                color = DietOneLetter, 
                                                shape = TypeOneLetter)) +
  labs(y = "clr abundance") +
  geom_vline(color = G_gry, size = G_vln, linetype = 1, xintercept = 1.5) +
  geom_point(size = G_pts, stroke = G_stk, alpha = 0.42, position = G_pos_jit) +
  geom_segment(inherit.aes = F, data = G_RvF_grp_6, size = G_seg,
               color = G_RvF_grp_5$color, linetype = G_RvF_grp_5$line, 
               aes(x = dodge + G_seg_scale, xend = dodge - G_seg_scale, 
                   y = med.clr, yend = med.clr)) +
  geom_segment(inherit.aes = F, data = G_RvF_grp_6, size = G_seg,
               color = G_RvF_grp_5$color, linetype = G_RvF_grp_5$line, 
               aes(x = dodge + G_seg_scale, xend = dodge - G_seg_scale, 
                   y = med.clr, yend = med.clr)) +
  scale_color_manual(values = c(G_hex_CFR[2], G_hex_CFR[3])) +
  scale_shape_manual(values = G_shp_typ) + 
  theme_pubr() +
  border(color = G_blk) +
  theme(line = element_line(lineend = "square"),
        text = element_text(family = G_fnt_fam),
        axis.text.y = element_text(size = G_axs_y, face = G_RvF_fce),
        axis.text.x = element_text(size = G_axs_x),
        axis.title.y = element_blank(),
        axis.title.x = element_text(size = G_ttl_x),
        legend.position = "none",
        panel.ontop = T,
        panel.background = element_rect(fill = NA)) +
  scale_x_continuous(breaks = G_RvF_xbrk, labels = G_RvF_xlab, 
                     expand = c(0.08, 0)) +
  scale_y_continuous(limits = G_lim_y, breaks = G_brk_y, labels = G_lab_y, 
                     expand = c(0, 0)) +
  coord_flip()
...
### ************************************
### G - STEP 4 - create ggplot legends ----
### ************************************

# create legend for sample type
# create df
G_lgn_shp <- data.frame(x = 1, y = 1, shape = c("E", "D", "P"))

# create ggplot
G_ggp_shp <- ggplot(data = G_lgn_shp, aes(x = x, y = y)) +
  geom_point(size = 1.4, stroke = 0.3, color = G_blk, aes(shape = shape)) +
  scale_shape_manual(values = c(G_shp_typ[2], G_shp_typ[1], G_shp_typ[3]),
                     name = NULL, labels = c("caecum", "colon (distal)", 
                                             "colon (proximal)")) +
  theme_void(base_family = G_fnt_fam) +
  theme(legend.position = c(0.395, 0.35),
        legend.direction = "vertical",
        legend.background = element_rect(fill = "white"),
        legend.key = element_rect(fill = NA, color = NA),
        legend.key.size = unit(0, "mm"),
        legend.text = element_text(size = 7, color = G_blk, hjust = 0,
                                   margin = margin(0.5, 0, 0.5, -0.9, 
                                                   "mm"))) +
  scale_x_continuous(limits = c(0, 2), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 2), expand = c(0, 0))
...
# create df
G_lgn_lne <- data.frame(x = c(0.9, 0.95, 0.9, 0.95, 0.9, 0.95), y = 1,
                        "color" = c("a", "b", "c"))

# create ggplot
G_ggp_lne <- ggplot(data = G_lgn_lne, aes(x = x, y = y, color = color)) +
  geom_line(size = 0.75, aes(linetype = color)) + 
...
### ************************************
### G - STEP 5 - arrange ggplots into panels ----
### ************************************

# arrange legends
G_gga_lgn <- ggpubr::ggarrange(G_ggp_lne, G_ggp_shp, ncol = 1, nrow = 2, 
                               align = "h")

# arrange plots
G_gga <- ggarrange(G_ggp_RvC, G_ggp_FvC, G_ggp_RvF, G_gga_lgn,
                   labels = c("A", "B", "C"), font.label = G_lab_fnt,
                   ncol = 2, nrow = 2, heights = c(1, 0.18), align = "hv")
...
# relative paths from wd for outputs headed for storage in "supplement/"
ggsave(device = "pdf", dpi = 600, units = "mm", width = 165, height = 230,
       filename = ofs_hth_EDP_alx_clr_plot, plot = G_gga)

# relative paths for outputs headed for storage in the "vault/"
ggsave(device = "pdf", dpi = 600, units = "mm", width = 165, height = 230,
       filename = G_ofv_hth_EDP_alx_clr_plot, plot = G_gga)

# create a list of objects from section and save them as a workspace file
G_obj <- c(ls(pattern = "E_"), "D_hth_EDP_CvR_q1_sigs_0",
           "D_hth_EDP_CvF_q1_sigs_0", "D_hth_EDP_RvF_q1_sigs_0")
save(list = G_obj, file = G_ofv_WS)
